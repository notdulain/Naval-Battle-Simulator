...
void BtoE(EscortShip Eships[], int eships, BattleShip Bship, int dx, int dy, int *targets){
...
void EtoB(EscortShip Eships[], int eships, BattleShip Bship, float *damage, int *status){
  ...
  for(int i=0; i<eships; i++){
    ...
    //if the battleship falls within that range, consider it's been attacked
    if(EtoB_distance <= upperlimit && EtoB_distance >= lowerlimit){
      *damage = *damage + Eships[i].impactpower;

      //if the damage exceeds 100%, consider the battleship destroyed and break from the loop
      if(*damage>=1){
        *status = 1;
        break;
      }
...
void battlefield(EscortShip Eships[], BattleShip Bship, int dx, int dy, int eships, int **canvas, int Bstatus){
  for(int i = 0; i < dy; i++){
    for(int j = 0; j < dx; j++){
...
void simulation(BattleShip battleships[], EscortShip escortships[]){
  ...
  battlefield(Eships, Bship, dx, dy, eships, canvas, Bstatus);

  BtoE(Eships, eships, Bship, dx, dy, &Eshipsinrange);
  EtoB(Eships, eships, Bship, &Bdamage, &Bstatus);
  ...
  if(Bstatus == 1){
    fprintf(fileptr, "Battleship gets destroyed in this simulation\n");
...
int main(){
  ...
  {"E_E", "Japanese Kaibokan", "(4.7 inch) naval guns", 0.04, 70, randomnumber(5, 15), randomnumber(1, 3)}};
