...
//A structure to store the status of a ship (Used for Eships)
typedef struct{
  int status;
}Status;
...
void BtoE(EscortShip Eships[], int eships, BattleShip Bship, int dx, int dy, PathXY path, Status Estatus[]){
...
void BtoE_jammed(EscortShip Eships[], int eships, BattleShip Bship, int dx, int dy, PathXY path, Status Estatus[]){
  ...
  for(int i = 0; i < eships; i++){
    ...
    if(Estatus[i].status != 1){
      //since the battleship gun is jammed, it has a similar type of range of an escortship
      ...
      if(BtoE_distance <= upperlimit && BtoE_distance >= lowerlimit){
        Estatus[i].status = 1; //if an Escortship falls within that range, consider it's destroyed
        usleep(Bship.gunfiretime); //the time between 2 consecutive gunfires
      }
    }
  }
...
void EtoB(EscortShip Eships[], int eships, BattleShip Bship, int *id, int *status, PathXY path, Status Estatus[]){
  ...
  for(int i=0; i<eships; i++){
    ...
    if(Estatus[i].status != 1){
      ...
      if(EtoB_distance <= upperlimit && EtoB_distance >= lowerlimit){
        *status = 1; //if the battleship falls within that range, consider it's been destroyed
        *id = Eships[i].id; //store the id of the Eship that attacked the Bship
      }
    } 
  }
}
...
void battlefield1(EscortShip Eships[], BattleShip Bship, int dx, int dy, int eships, int **canvas, int Bstatus, PathXY path[], int points){
  ...
  for(int i = 0; i < eships; i++){
    canvas[dy - Eships[i].y][Eships[i].x - 1] = Eships[i].id;  //assign Eships' ID's
  }

  for(int i = 1; i < points; i++){
    canvas[dy - path[i].y][path[i].x - 1] = -2; //assign -2 for the Bship's path
  }
  ...
  for(int i=0; i<dy; i++){
    for(int j=0; j<dx; j++){
      if(canvas[i][j] == 0){
        ...
        printf("P  "); //print P for the path
      }
      else if(canvas[i][j]>=10){
        printf("%d ", canvas[i][j]); //print E ship ID's with double digits
      }
      else{
        printf("%d  ",canvas[i][j]); //print E ship ID's with a single digit
      }
    }
    printf("\n\n");
  }
...
void battlefield2(EscortShip Eships[], BattleShip Bship, int dx, int dy, int eships, int **canvas, int Bstatus, PathXY path[], int points, int count, int Bx, int By, Status Estatus[]){
  for(int i = 0; i < dy; i++){
    for(int j = 0; j < dx; j++){
        canvas[i][j] = 0; //assign 0 for all the points on the canvas
    }
...
void simulation(BattleShip battleships[], EscortShip escortships[]){
  ...
  for(int i=0; i<jammedin; i++){
    ...
    BtoE(Eships, eships, Bship, dx, dy, path[i], Estatus);
    EtoB(Eships, eships, Bship, &This_E_hit_B ,&Bstatus, path[i], Estatus);  
  ...
  for(int i=jammedin; i<points; i++){
    ...
    BtoE_jammed(Eships, eships, Bship, dx, dy, path[i], Estatus);
    EtoB(Eships, eships, Bship, &This_E_hit_B ,&Bstatus, path[i], Estatus);
  ...
  battlefield2(Eships, Bship, dx, dy, eships, canvas, Bstatus, path, points, count, Bx, By, Estatus);
