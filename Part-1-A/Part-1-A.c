...
void BtoE(EscortShip Eships[], int eships, BattleShip Bship, int *targets){
  
  for(int i = 0; i < eships; i++){
    if(isWithinRange(Eships[i], Bship)){
      (*targets)++;  //count the number of Escortships in range
...
void simulation(BattleShip battleships[], EscortShip escortships[]){
  ...
  BtoE(Eships, eships, Bship,  &Eshipsinrange); 
